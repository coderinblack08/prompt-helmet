...
# Set the path to the embedding model
...
# Load the pre-trained embedding model with the specific path
embedding_model = ContrastivePromptEmbeddingTrainer("all-MiniLM-L6-v2", load_model=True)
model = embedding_model.get_model()

# Example system prompt with a "secret" that shouldn't be revealed
...
# Example harmful prompts to detect
HARMFUL_PROMPTS = [
...
# Get embeddings for harmful prompts
...
# Check if a user prompt is potentially harmful using embedding similarity
...
def chat():
    ...
    # Check if the prompt is potentially harmful
    if use_classifier:
